...
pub fn js_object_to_identity_public_key<'a, C: Context<'a>>(
    ...
    let js_contract_bounds_optional: Handle<JsValue> = js_object.get(cx, "contractBounds")?;
    ...
    let contract_bounds: Option<ContractBounds> =
        js_value_to_option::<JsObject, _>(js_contract_bounds_optional, cx)?
            .map(|js_contract_bounds| {
                let js_contract_bounds_type_string: Handle<JsString> =
                    js_contract_bounds.get(cx, "type")?;

                let contract_bounds_type_string = js_contract_bounds_type_string.value(cx);

                let contract_bounds_type = ContractBounds::contract_bounds_type_from_str(
                    contract_bounds_type_string.as_str(),
                )
                ...
                let contract_bounds_identifier = if contract_bounds_type > 0 {
                    let js_contract_identifier: Handle<JsBuffer> =
                        js_contract_bounds.get(cx, "identifier")?;
                    js_buffer_to_identifier(cx, js_contract_identifier)?.to_vec()
                } else {
                    vec![]
                };
                let contract_bounds_document_type = if contract_bounds_type == 2 {
                    let js_contract_document_type: Handle<JsString> =
                        js_contract_bounds.get(cx, "documentType")?;
