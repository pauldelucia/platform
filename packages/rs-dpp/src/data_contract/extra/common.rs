...
/// Gets the inner u8 value from cbor map
pub fn cbor_inner_u8_value(document_type: &[(Value, Value)], key: &str) -> Result<Option<u8>, ProtocolError> {
    let key_value = get_key_from_cbor_map(document_type, key);
    let Some(key_value) = key_value else {
        return Ok(None)
    };
    if let Value::Integer(int) = key_value {
